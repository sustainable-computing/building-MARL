...
if __name__ == '__main__':
    ...
    os.makedirs(f"policy_library/evaluation/{args.evaluate_on}-{args.evaluate_start}-{args.evaluate_length}", exist_ok=True)
    model = Model(idf_file_name="./eplus_files/OfficeMedium_Denver.idf",
                  ...
                  tmp_idf_path=f"policy_library/evaluation/{args.evaluate_on}-{args.evaluate_start}-{args.evaluate_length}")
    log_f = open(f"policy_library/evaluation/{args.evaluate_on}-{args.evaluate_start}-{args.evaluate_length}-log.csv", "w+")
    ...
    # Environment setup
    model.set_runperiod(*(90, 1990, 1, 1))
