...
for seed in ${SEEDS[@]} ; do
    for train_zone in ${TRAIN_ON[@]} ; do
        tmux new-session -d -s "bash-session-$train_zone$seed" "conda activate layoutsim; python3 main.py --gpu $gpu --seed $train_zone$seed --multi_agent 1 --train_on $train_zone --prefix new_experiment/"
        let gpu=(gpu+1)%3
    done
done
